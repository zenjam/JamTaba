...
IMAGE_NAME="${IMAGE_NAME:-jamtaba-linux-build}"
PLATFORM="${DOCKER_PLATFORM:-linux/amd64}"
LOG_FILE="${LOG_FILE:-/tmp/jamtaba-linux-docker-build.log}"
RUN_LOG_FILE="${RUN_LOG_FILE:-/tmp/jamtaba-linux-container-build.log}"
VERBOSE="${VERBOSE:-0}"
...
echo "Running JamTaba Linux build inside container"
...
if [ "${RUN_STATUS}" -ne 0 ]; then
    echo
    echo "JamTaba Linux build failed. First likely error lines:"
...
echo "JamTaba Linux build finished"
echo "Container build time: $(format_duration "$(( $(date +%s) - RUN_START_TIME ))")"
echo "Total elapsed: $(format_duration "$(( $(date +%s) - SCRIPT_START_TIME ))")"
