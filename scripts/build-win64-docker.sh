...
IMAGE_NAME="${IMAGE_NAME:-jamtaba-win64-build}"
PLATFORM="${DOCKER_PLATFORM:-linux/amd64}"
LOG_FILE="${LOG_FILE:-/tmp/jamtaba-win64-docker-build.log}"
RUN_LOG_FILE="${RUN_LOG_FILE:-/tmp/jamtaba-win64-container-build.log}"
VERBOSE="${VERBOSE:-0}"
...
if [ "${VERBOSE}" = "1" ]; then
    docker build \
        ...
        --progress=plain \
        -t "${IMAGE_NAME}" \
        -f "${ROOT_DIR}/docker/win64/Dockerfile" \
...
echo "Running JamTaba win64 build inside container"
...
if [ "${RUN_STATUS}" -ne 0 ]; then
    echo
    echo "JamTaba win64 build failed. First likely error lines:"
...
echo "JamTaba win64 build finished"
echo "Container build time: $(format_duration "$(( $(date +%s) - RUN_START_TIME ))")"
echo "Total elapsed: $(format_duration "$(( $(date +%s) - SCRIPT_START_TIME ))")"
